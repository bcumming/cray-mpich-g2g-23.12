...
int main(int argc, char** argv) {
    ...
    if (rank==0) {
        ...
        if (snd_loc=='H') {
            check_mpi_status(
                MPI_Send(hbuffer, N, MPI_BYTE, 1, tag, MPI_COMM_WORLD));
        }
        else {
            check_cuda_status(
                cudaMemcpy(dbuffer, hbuffer, N, cudaMemcpyHostToDevice));
            check_mpi_status(
                MPI_Send(dbuffer, N, MPI_BYTE, 1, tag, MPI_COMM_WORLD));
        }
    }
    else {
        for (int i=0; i<N; ++i) hbuffer[i] = 0;
        MPI_Status status_;
        if (rcv_loc=='H') {
            check_mpi_status(
                MPI_Recv(hbuffer, N, MPI_BYTE, 0, tag, MPI_COMM_WORLD, &status_));
        }
        else {
            check_mpi_status(
                MPI_Recv(dbuffer, N, MPI_BYTE, 0, tag, MPI_COMM_WORLD, &status_));
            check_cuda_status(
